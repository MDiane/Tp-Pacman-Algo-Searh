...
def cornersHeuristic(state, problem):
    ...
    while True:
        if notCornersV.isEmpty() == True:
            break
        
        cornerDistance = []
        for c in notCornersV.list:
            cornerDistance.append([util.manhattanDistance(state, c), c])
        
        state = min(cornerDistance)[1]
        notCornersV.list.remove(state)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        while True:
            ...
            for i in reversed( range(0, len (successors) ) ):
                if successors[i][0] not in visited.list:
                    surround.push([successors[i][0], actions + [ successors[i][1] ] ])
                    visited.push(successors[i][0])
